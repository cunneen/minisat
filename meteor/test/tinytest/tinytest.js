...
const { Meteor, __meteor_runtime_config__, globalThis, EJSON, Random } = require("./MeteorStubs")
const { check, Match } = require("./check")
/******************************************************************************/
/* TestCaseResults                                                            */
/******************************************************************************/
...
class TestCaseResults {
  ...
  fail(doc) {
    ...
    this.onEvent({
      ...
      details: doc,
      cookie: {
        name: this.test_case.name, offset: this.current_fail_count,
  ...
  equal(actual, expected, message, not) {
    ...
    if (typeof expected === "object" && expected && expected.nodeType) {
      ...
      if (!(actual instanceof Uint8Array))
        this.fail({
          type: "assert_equal", message: "found object is not a typed array",
          expected: "A typed array", actual: actual.constructor.toString()
        });
      if (expected.length !== actual.length)
        this.fail({
          type: "assert_equal", message: "lengths of typed arrays do not match",
    ...
    if (matched === !!not) {
      this.fail({
        type: "assert_equal", message: message,
...
module.exports = { Tinytest, EJSON, Random, Match, check, Meteor };